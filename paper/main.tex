...
\begin{tikzpicture}[font=\footnotesize, node distance=7mm]
  \node[draw, rounded corners, fill=tablerowgray, text width=0.88\textwidth, align=left, inner sep=6pt] (gov)
  {\textbf{Governance plane} (policy intent): REQ catalogue, role and recusal constraints, consent/warrant rules, conformance profile declarations.};
  \node[draw, rounded corners, fill=white, text width=0.88\textwidth, align=left, inner sep=6pt, below=of gov] (run)
  {\textbf{Runtime enforcement plane} (execution gates): mode classifier, replay authorisation gate, override timer/checkpoint controller, suspension trigger, dispute intake routing.};
  \node[draw, rounded corners, fill=tablerowgray, text width=0.88\textwidth, align=left, inner sep=6pt, below=of run] (evd)
  {\textbf{Evidence and assurance plane} (audit outputs): signed decisions, warrants, digest-chain entries, case packets, closure bundles, release test reports.};
  \draw[-{Latex[length=2.2mm,width=1.4mm]}, thick] (gov.south) -- (run.north);
  \draw[-{Latex[length=2.2mm,width=1.4mm]}, thick] (run.south) -- (evd.north);
  \draw[-{Latex[length=2.2mm,width=1.4mm]}, thick, bend left=12] (evd.east) to node[right, xshift=2mm] {audit feedback} (gov.east);
\end{tikzpicture}
