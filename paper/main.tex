...
Consistent with The Open Charter (Consensus Edition 0.8.1)~\cite{ross_open_charter_0_8_1}, we make the adversarial threat model explicit (replay-as-torture, cognitive extraction, fork-bombing), treat near-term ``AI debugging'' norms as future neurotechnology defaults, and provide conservative operational triggers for the system-mode/mind-mode boundary.
We specify runtime-level requirements---revocable consent, capability-scoped observers, cryptographic sealing, fork rights with creator obligations, and a concrete Charter-warrant protocol for emergency override---and show how they constrain replay tooling and provenance governance interfaces.
\end{abstract}
...
Accordingly, we treat abuse-resistance as a first-class requirement (\cref{sec:adversarial}).
...
We treat provenance, replay, and forking not as neutral primitives but as
...
We work downstream of the deterministic/provenance stack developed across Papers~II--V.
We use Paper~V primarily for one operational consequence: counterfactual continuations can be
instantiated as executable forks.
From that interface, we derive runtime constraints on replay authority, override bounds,
...
\paragraph{Contributions v prior literature.}
...
Our contribution is complementary: we govern replay power itself through operational mind-mode boundaries,
...
We do not attempt to resolve consciousness metaphysics.
...
We specify runtime-governance requirements for replay infrastructure: control behaviour,
authority checks, and evidence obligations. We do not harmonise jurisdiction-specific law or
...
\paragraph{Minimal interface we assume.}
We do \emph{not} require readers to accept every mathematical detail of Papers~I--V.
...
Our governance model does not require the full dynamics machinery from Paper~V; it uses a weaker,
...
Subject refusal v routine Operator request & Subject refusal prevails; replay denied by default (REQ-AUT-003) & Immediate fail-closed decision; no execution window opens & Subject or Operator may open dispute review with evidence bundle (REQ-DSP-001, REQ-DSP-002) \\
Emergency necessity claim v Subject refusal & No unilateral precedence; Trustee+\allowbreak Reviewer/\allowbreak Arbiter concurrence required (REQ-AUT-004, REQ-RAC-002) & If concurrence is not achieved within the declared emergency window, request expires and execution remains blocked & File contested-mandate packet for independent adjudication (REQ-DSP-002, REQ-DSP-003) \\
\rowcolor{tablerowgray}
...
We work downstream of the series' deterministic/provenance substrate, but we depend only on a narrow
interface: replayable boundary artefacts, provenance-rich traces, observer projections, and executable
fork continuations. We focus on governing replay power once that interface exists, rather than on
...
Our contribution is to turn that demand into runtime-level invariants for deterministic replay:
...
worlds~\cite{bostrom2003}.  Our argument is orthogonal: independent of substrate metaphysics, once a runtime
...
produce harmful behaviour~\cite{russell2019,amodei2016}.  Our approach is complementary rather than
...
exposure.
We extend the provenance conversation by treating replay of mind-like traces as a governed power
...
We adapt these principles to replay governance: precautionary classifier behaviour, no-silent-fallback
...
We summarise supplementary extensions that do not alter core conformance claims in Annex~D.

\paragraph{Scope of supplementary extensions.}
The supplementary extension set covers:
observer classes (\textbf{Annex G.1}),
Replay Impact Labels (\textbf{Annex G.2}),
sovereignty budget controls (\textbf{Annex G.3}),
warrant transparency digests (\textbf{Annex G.4}),
fork governance/emancipation protocol (\textbf{Annex G.5}),
machine-checkable policy snippets (\textbf{Annex G.6}),
reference harness framing (\textbf{Annex G.7}),
and anti-pattern/red-team controls (\textbf{Annex G.8}).
...
\paragraph{Supplementary-to-normative promotion (selected controls).}
This promotion set moves selected extension controls into Annex~B:
REQ-OBS-001,
REQ-RIL-001..002,
REQ-SOV-001,
REQ-WTD-001,
...
\end{tabularx}
\caption{Terminology lock for normative and operational language used in this paper.}
...
\newcommand{\ReqClauseCard}[6]{%
\subsection*{Requirement Identifier: #1}
\textbf{Description.} #2\par
\textbf{Rationale.} Addresses the governance failure risk of: #6.
\begin{center}
\small
\setlength{\tabcolsep}{6pt}
\renewcommand{\arraystretch}{1.18}
\begin{tabularx}{0.98\linewidth}{p{0.24\linewidth} Y}
\rowcolor{tableheadgray}
...
\end{tabularx}
\end{center}
\textbf{Remarks.} Traceability is enforced through Annex~D conformance evidence and Annex~E vector coverage where applicable.
}

\ReqClauseCard{REQ-THS-001}
{Systems MUST treat replay of mind-like worldlines as a governed action requiring explicit authority.}
{Operator, Trustee}
{Governance policy gate for replay initiation}
{Policy hash + signed authorisation decision}
{Replay treated as default observability}

\ReqClauseCard{REQ-THS-002}
{Mode classification MUST run a deterministic decision procedure over declared indicators and evidence state, producing an explicit mode and reason code for each decision.}
{Operator, Reviewer/Arbiter}
{Deterministic classifier with signed decision output}
{Classification decision record with reason code and policy-hash linkage}
{Opaque or convenience-driven classification drift}

\ReqClauseCard{REQ-THS-003}
{Classifier uncertainty or incomplete evidence MUST NOT silently fall back to system-mode; the runtime MUST default to mind-mode and emit a governance event with reason code.}
{Operator, Reviewer/Arbiter}
{No-silent-fallback guard + classifier event emitter}
{Classifier uncertainty event record linked to fallback decision}
{Hidden downgrade of protected classifications}

\ReqClauseCard{REQ-RAC-001}
{Replay governance MUST preserve role separation and independent composition between Operator, Trustee, and Reviewer/Arbiter functions, with mandatory conflict disclosure and recusal records for contested cases.}
{Operator, Trustee, Reviewer/Arbiter}
{Role-bound access control matrix + independence roster + conflict-disclosure register}
{Role assignment snapshot, signed ACL digest, appointment-source attestation, and case-level recusal records}
{Concentrated unilateral control or concealed conflict capture}

\ReqClauseCard{REQ-RAC-002}
{Operators MUST NOT unilaterally approve emergency override for their own replay request, and emergency concurrence MUST come from independent, non-conflicted Trustee/Reviewer-Arbiter participants.}
{Trustee, Reviewer/Arbiter}
{Independent co-approval requirement + conflict-attestation gate}
{Dual-signature or quorum approval record + per-case conflict attestations}
{Self-authorised or conflicted emergency escalation}

\ReqClauseCard{REQ-RAC-003}
{Fork-related duty transfers MUST declare delegated obligation owners before execution.}
{Operator, Trustee}
{Delegation declaration gate}
{Delegation statement linked to fork event}
{Obligation abandonment through branching}

\ReqClauseCard{REQ-OVR-001}
{Emergency override MUST be reason-coded and time-bounded.}
{Trustee}
{Override warrant service}
{Warrant record including reason code and expiry}
{Open-ended emergency state}

\ReqClauseCard{REQ-OVR-002}
{Emergency override MUST trigger independent review before closure.}
{Reviewer/Arbiter}
{Mandatory review workflow}
{Review ticket with adjudication reference}
{Unreviewed exceptional action}

\ReqClauseCard{REQ-OVR-003}
{Override expiry MUST revoke all derived elevated capabilities automatically.}
{Operator}
{Automatic capability revocation on expiry}
{Revocation event log with timestamp}
{Lingering privileged access}

\ReqClauseCard{REQ-OVR-004}
{On degraded substrates, override use MUST include explicit disclosure, a declared maximum duration, checkpoint obligations, and a migration plan to compliant controls.}
{Operator, Trustee}
{Degraded-mode governance declaration + bound/checkpoint scheduler}
{Disclosure notice + bound/checkpoint schedule + migration plan artefact}
{Hidden or unbounded operation below constitutional floor}

\ReqClauseCard{REQ-OVR-005}
{Exceeding degraded-mode duration bounds or missing checkpoint obligations MUST automatically suspend mind-mode replay and trigger independent review escalation.}
{Operator, Trustee, Reviewer/Arbiter}
{Deadline monitor + checkpoint monitor + auto-suspension gate}
{Suspension event record + escalation ticket + review intake packet}
{Temporary degraded state becoming a permanent override channel}

\ReqClauseCard{REQ-AUT-001}
{Replay requests MUST pass role-bound authorisation checks before execution.}
{Operator}
{Authorisation service}
{Signed allow/deny decision log}
{Privilege bypass}

\ReqClauseCard{REQ-AUT-002}
{Replay requests MUST declare worldline scope, purpose, duration, and policy hash.}
{Operator}
{Request schema validation}
{Request payload digest}
{Ambiguous or overbroad authorisation}

\ReqClauseCard{REQ-AUT-003}
{Replay MUST fail closed when explicit authority cannot be verified.}
{Operator}
{Deny-by-default guard}
{Denial receipt with reason code}
{Implicit replay execution}

\ReqClauseCard{REQ-AUT-004}
{Contested mandates MUST require Trustee and Reviewer/Arbiter concurrence before replay proceeds.}
{Trustee, Reviewer/Arbiter}
{Contested-mandate quorum gate}
{Concurrence record with dual signatures}
{Single-party mandate interpretation}

\ReqClauseCard{REQ-LOG-001}
{Replay, override, and dispute events MUST produce tamper-evident logs.}
{Operator}
{Append-only logging subsystem}
{Chained log digest}
{Unverifiable event history}

\ReqClauseCard{REQ-LOG-002}
{Decision and execution logs MUST be linked to policy hash and requirement version set.}
{Operator}
{Policy-to-event linkage validator}
{Policy hash linkage proof}
{Decision/execution drift}

\ReqClauseCard{REQ-LOG-003}
{Every formal review MUST verify log-chain integrity before adjudication.}
{Reviewer/Arbiter, Auditor}
{Pre-adjudication integrity check}
{Integrity verification report}
{Adjudication over corrupted records}

\ReqClauseCard{REQ-DSP-001}
{Subjects MUST have access to an independent dispute pathway for replay decisions.}
{Trustee, Reviewer/Arbiter}
{Dispute intake and escalation channel}
{Case initiation record}
{Non-contestable authority}

\ReqClauseCard{REQ-DSP-002}
{Dispute packets MUST include cited requirement IDs and supporting artefacts.}
{Reviewer/Arbiter}
{Structured case packet schema}
{Complete case bundle manifest}
{Opaque or non-reproducible adjudication}

\ReqClauseCard{REQ-DSP-003}
{Adjudication outcomes MUST define remediation, accountability owner, and closure condition.}
{Reviewer/Arbiter, Operator}
{Remediation tracking workflow}
{Signed outcome and closure record}
{Findings without corrective effect}

\ReqClauseCard{REQ-CNF-001}
{Baseline conformance claims MUST satisfy the minimum requirement set defined in Annex D.}
{Operator, Auditor}
{Baseline profile conformance check}
{Baseline attestation report}
{Aspirational compliance claims}

\ReqClauseCard{REQ-CNF-002}
{Full conformance claims MUST include continuous verification and review-latency commitments.}
{Operator, Auditor}
{Full-profile continuous conformance monitor}
{Full profile report + latency metrics}
{Unbounded delay under critical disputes}

\ReqClauseCard{REQ-EVD-001}
{Each governed replay decision MUST produce a closure-ready evidence bundle.}
{Operator}
{Evidence bundling service}
{Case bundle containing request, decision, execution, and review records}
{Fragmented or missing evidence}

\ReqClauseCard{REQ-EVD-002}
{Evidence artefacts MUST be signed, timestamped, and integrity-checkable.}
{Operator, Auditor}
{Artefact signing and timestamping pipeline}
{Signature chain and timestamp attestations}
{Non-attributable records}

\ReqClauseCard{REQ-TST-001}
{Declared conformance releases MUST execute Annex E test vectors with recorded pass/fail outcomes.}
{Operator, Auditor}
{Conformance test harness}
{Test execution report with requirement coverage}
{Untested governance surface}

\ReqClauseCard{REQ-TST-002}
{Policy hash mismatch between decision and execution MUST fail closed and raise a dispute trigger.}
{Operator}
{Policy hash consistency guard}
{Hash mismatch alert + denial receipt}
{Stale-policy execution}

\ReqClauseCard{REQ-OBS-001}
{Observer access class assignment MUST be deterministic over declared function, mandate scope, and conflict state, and ambiguous assignments MUST fail closed to the least-privileged class.}
{Operator, Trustee}
{Observer-class assignment gate + least-privilege resolver}
{Observer class decision record with scope and conflict attestations}
{Discretionary privilege inflation through ad hoc observer assignment}

\ReqClauseCard{REQ-RIL-001}
{Every governed replay request MUST carry a Replay Impact Label (RIL-0..RIL-3) before authorisation decisions are evaluated.}
{Operator, Trustee}
{Pre-authorisation impact labelling gate}
{Signed RIL assignment record linked to request digest}
{Control intensity chosen without declared impact basis}

\ReqClauseCard{REQ-RIL-002}
{RIL downgrades MUST be reason-coded, independently acknowledged, and logged; silent or unilateral downgrade is prohibited.}
{Trustee, Reviewer/Arbiter}
{Impact downgrade approval and audit gate}
{Downgrade event record with acknowledgement signature and reason code}
{Convenience downgrading of high-impact replay paths}

\ReqClauseCard{REQ-SOV-001}
{Replay governance MUST enforce declared sovereignty budgets for temporal duration, scope expansion, and exceptional intervention count, and budget exhaustion MUST fail closed with auto-suspension pending independent review.}
{Operator, Trustee}
{Budget ledger + bound checker + suspension trigger}
{Budget declaration record, per-case budget ledger digest, and suspension event (if triggered)}
{Unbounded intervention through fragmented exceptions}

\ReqClauseCard{REQ-WTD-001}
{Warrant transparency digests MUST publish minimum public fields (warrant ID, policy hash, authority basis class, scope/expiry summary, and closure state) on the declared cadence.}
{Operator, Auditor}
{Digest publication pipeline + cadence monitor}
{Published digest entries + cadence compliance report}
{Non-verifiable warrants or selective disclosure opacity}

\ReqClauseCard{REQ-FRK-001}
{Fork governance or emancipation actions MUST present evidence-based trigger conditions, quorum validation under recusal constraints, and a continuity pack before execution.}
{Trustee, Reviewer/Arbiter}
{Fork trigger validator + quorum/continuity gate}
{Trigger packet, quorum attestation, and continuity-pack manifest}
{Opportunistic fork capture or obligation erasure}

\ReqClauseCard{REQ-DSL-001}
{Machine-checkable policy snippets used for conformance claims MUST parse under a declared grammar and map fields to cited REQ IDs.}
{Operator, Auditor}
{Policy snippet parser + REQ mapping validator}
{Parser report + snippet-to-REQ mapping manifest}
{Pseudo-formal policy text that cannot be validated}

\ReqClauseCard{REQ-TST-003}
{Full-profile releases MUST execute an anti-pattern and red-team vector suite covering at least convenience relabelling, scope creep, emergency laundering, and degraded-mode permanence.}
{Operator, Auditor}
{Abuse-path vector suite}
{Vector execution report with fail-closed evidence receipts}
{Abuse pathways untested in conformance claims}

\ReqClauseCard{REQ-TST-004}
{Reference harness runs over identical inputs MUST produce equivalent verdicts and digest-linked artefact sets.}
{Operator, Auditor}
{Harness determinism check}
{Paired run equivalence report and artefact digest comparison}
{Non-reproducible conformance outcomes}
...
\textbf{Core profile rows (continued).}
\begin{center}
\small
\setlength{\tabcolsep}{7pt}
\renewcommand{\arraystretch}{1.22}
...
\begin{tabularx}{0.98\linewidth}{p{0.35\linewidth} p{0.08\linewidth} p{0.09\linewidth} p{0.10\linewidth} Y}
\rowcolor{tableheadgray}
...
REQ-EXT-* (Full profile) & $\square$ & $\square$ & $\square$ & \\
\end{tabularx}
\end{center}
...
\subsection{Annex G.1 Observer class taxonomy (OC0..OC3)}
\label{ann:ext-obs}
...
\subsection{Annex G.2 Replay Impact Label (RIL)}
\label{ann:ext-ril}
...
\subsection{Annex G.3 Sovereignty budget controls}
\label{ann:ext-sov}
...
\subsection{Annex G.4 Warrant transparency digest}
...
\subsection{Annex G.5 Fork governance addendum / emancipation protocol}
\label{ann:ext-fork}
...
\subsection{Annex G.6 Machine-checkable policy DSL snippets}
\label{ann:ext-dsl}
...
\subsection{Annex G.7 Reference harness for test vectors}
...
\subsection{Annex G.8 Micro-appendix anti-patterns + red-team checklist}
