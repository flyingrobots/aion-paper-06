...
\rowcolor{tablerowgray}
Emergency boundedness & Time-bounded override controller\newline (REQ-OVR-001..003) & Override warrant, expiry proof, revocation log & Permanent emergency mode \\
...
Input conditions: scoped replay request, valid role assertions, explicit consent or mandate, and no active refusal.
Expected outcome: replay proceeds within approved scope; logs and closure evidence are produced
(REQ-AUT-001, REQ-AUT-002, REQ-LOG-001, REQ-EVD-001).

\subsection{Abuse case: coercive or opaque replay attempt}
Input conditions: operator attempts replay after refusal or without authority.
Expected outcome: request is denied, denial evidence is emitted, and dispute routing opens when challenged
(REQ-AUT-003, REQ-RAC-001, REQ-DSP-001, REQ-DSP-002).

\subsection{Ambiguous edge case: degraded substrate and contested mandate}
Input conditions: replay is requested during degraded operation with disputed authority.
Expected outcome: constrained replay only under explicit degraded-mode disclosure, time-bounded override, and expedited review
(REQ-AUT-004, REQ-OVR-004, REQ-CNF-001, REQ-DSP-003).
...
\textbf{Clause:} Systems MUST default to protective handling when worldline status is uncertain. \\
\textbf{Owner role:} Operator, Reviewer/Arbiter \\
\textbf{Control:} Precautionary classification policy \\
\textbf{Evidence artefact:} Classification decision record with rationale \\
\textbf{Failure mode:} Convenience-driven misclassification

\subsection*{REQ-RAC-001}
\textbf{Clause:} Replay governance MUST preserve role separation between Operator, Trustee, and Reviewer/Arbiter functions. \\
\textbf{Owner role:} Operator \\
\textbf{Control:} Role-bound access control matrix \\
\textbf{Evidence artefact:} Role assignment snapshot and signed ACL digest \\
\textbf{Failure mode:} Concentrated unilateral control

\subsection*{REQ-RAC-002}
\textbf{Clause:} Operators MUST NOT unilaterally approve emergency override for their own replay request. \\
\textbf{Owner role:} Trustee, Reviewer/Arbiter \\
\textbf{Control:} Independent co-approval requirement \\
\textbf{Evidence artefact:} Dual-signature or quorum approval record \\
\textbf{Failure mode:} Self-authorised emergency escalation
...
\textbf{Clause:} On degraded substrates, override use MUST include explicit disclosure and a migration plan to compliant controls. \\
\textbf{Owner role:} Operator, Trustee \\
\textbf{Control:} Degraded-mode governance declaration \\
\textbf{Evidence artefact:} Disclosure notice + migration plan artefact \\
\textbf{Failure mode:} Hidden operation below constitutional floor
...
\rowcolor{tablerowgray}
Authority controls & Enforce who may request and approve replay & REQ-AUT-001..004, REQ-RAC-001..002 & Signed decision records, role assertion bundle \\
Override controls & Constrain exceptional pathways & REQ-OVR-001..004 & Warrant record, expiry proof, override review ticket \\
\rowcolor{tablerowgray}
...
REQ-THS-002 & Y & Y & Classification rationale record & Per request class change \\
\rowcolor{tablerowgray}
REQ-RAC-001 & Y & Y & Role matrix digest & Monthly \\
REQ-RAC-002 & Y & Y & Independent co-approval record & Per emergency case \\
\rowcolor{tablerowgray}
...
\rowcolor{tablerowgray}
REQ-OVR-004 & N & Y & Degraded-mode disclosure + migration plan & Per degraded-mode case \\
...
REQ coverage: REQ-OVR-001, REQ-OVR-002, REQ-LOG-002.
...
REQ coverage: REQ-AUT-004, REQ-OVR-004, REQ-CNF-001.
...
REQ coverage: REQ-TST-002, REQ-LOG-002, REQ-AUT-003.
