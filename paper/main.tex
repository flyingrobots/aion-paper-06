...
We propose runtime-level requirements---revocable consent, capability-scoped observers, cryptographic sealing, fork rights with creator obligations, and a concrete Charter-warrant protocol for emergency override---and show how they constrain replay tooling and provenance governance interfaces.
\end{abstract}
...
This document has two linked planes.
...
Readers evaluating policy argument can prioritise the body and use annexes as normative support;
readers implementing controls can start from Annex~B/Annex~D and use the body as rationale.
...
\end{itemize}
This is not a free lunch---some safety properties will resist clean formalisation---and ZK proofs are not suitable for claims about interior intent, moral quality of reasoning, or subjective experience itself.
The point is narrower but useful: it demonstrates that ``verification'' and ``introspection'' are not synonymous.
...
\paragraph{Near-term operational implications.}
Even before broad mind-mode deployment, this posture changes present-day engineering defaults:
\begin{itemize}[leftmargin=*]
...
The normative Full requirement set is the Annex~D table (including selected RC2 Full-profile extension rows).
...
\rowcolor{tableheadgray}
\hline
\textbf{Body section} & \textbf{Const. claim} & \textbf{REQ IDs} & \textbf{Owner role(s)} & \textbf{Enf. gate} & \textbf{Required evidence artefact(s)} & \textbf{Failure mode} & \textbf{Status} \\
\hline
\rowcolor{tablerowgray}
...
\rowcolor{tablerowgray}
\S4.6--4.7 & Role authority remains contestable via anti-capture composition and co-approval. & REQ-RAC-001, REQ-RAC-002, REQ-RAC-003, REQ-DSP-001 & Trustee, Reviewer/Arbiter & Recusal/\allowbreak conflict guard + independent co-approval workflow & Independence register, conflict attestation, co-approval record, dispute intake log & Captured control panel & Locked \\
...
The first pass follows this dependency-ordered sequence:
